local custom = {
    ...
    system = {
        local_dir = (function()
            local nvim_dir = vim.fn.expand("$ST_HOME")
            if nvim_dir == "" then
                vim.api.nvim_echo({ { "Failed to find ST_HOME Path", "ErrorMsg" } }, true, {})
                os.exit(1)
            end
            local path = nvim_dir .. "/.cache/nvim/local/"
...
--> Setup plugins
require("lazy").setup({
    --> Language server binary installer only
    {
        "williamboman/mason.nvim",
        opts = {
            install_root_dir = custom.system.local_dir .. "/mason"
        }
    },
    ...
    --> Install and enable LSPs
    {
        ...
        config = function()
            local lsps = {
                -- "lua_ls@3.15.0", Pin versions like so
                "lua_ls",
                "rust_analyzer",
                "clangd",
                "bashls",
    ...
    --> Allows declarative management of non-lsp mason items
    {
